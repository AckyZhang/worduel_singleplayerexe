...
def play_wordle():
    ...
    root.update()
    wl = int(simpledialog.askstring("Input", "Word length to guess(5-7):", parent=root))
    if wl < 5:
        messagebox.showinfo("Wrong Number", f"{wl} is too low. The minimum number of word length to guess is 5. The game will start with 5-letter words.")
        wl = 5
    elif wl > 7:
        messagebox.showinfo("Wrong Number", f"{wl} is too high. The maximum number of word length to guess is 7. The game will start with 7-letter words.")
        wl = 7
    center_window(root)
    ...
    center_window(root)
    if attempts < 5:
        messagebox.showinfo("Wrong Number", f"{attempts} is too low. The minimum number of attempts is 5. The game will start with 5 attempts")
        attempts = 5
    elif attempts > 15:
        messagebox.showinfo("Wrong Number", f"{attempts} is too high. The maximum number of attempts is 15. The game will start with 15 attempts")
        attempts = 15
    attempts = [attempts]
